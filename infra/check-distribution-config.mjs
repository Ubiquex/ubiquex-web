...
if (!comment.startsWith("ubiquex-web")) {
  problems.push(
    `Comment must start with "ubiquex-web": the idempotency lookup filters on that prefix`,
  );
}
...
// out/404/index.html, not out/404.html. The docs sites are the opposite.
// Getting this wrong points the error page at a file that does not exist,
// which shows as a bare CloudFront error rather than the site's 404.
const ERROR_PAGE = "/404/index.html";
...
for (const code of [403, 404]) {
  ...
  } else if (entry.ResponsePagePath !== ERROR_PAGE || entry.ResponseCode !== "404") {
    problems.push(
      `CustomErrorResponse ${code} should serve ${ERROR_PAGE} as 404, ` +
